        ...
        while True:
            ...
            try:
                ...
                if err.code == 429 and attempt < max_attempts:
                    # Yahoo Finance occasionally rate limits the CSV endpoint when
                    # too many requests are made in quick succession. Retry with a
                    # simple exponential backoff so users are less likely to hit the
                    # limit when invoking the CLI repeatedly.
                    time.sleep(backoff_seconds)
                    backoff_seconds *= 2
                    continue
                if err.code == 429:
                    raise ValueError(
                        "Failed to download data due to Yahoo Finance rate limiting (HTTP 429). "
                        "Please wait a moment and retry, or provide cached data via --data-file."
                    ) from err
                raise ValueError(
